...
namespace BMT_PollutedLands
{
	...
	public class CompProperties_AbilityCorpseEater : CompProperties_AbilityEffect
	{
		...
		public float minNutritionGain = 0.2f;
	...
	public class CompAbilityEffect_CorpseEater : CompAbilityEffect
	{
		...
		public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
		{
			...
			if (target.HasThing && target.Thing is Corpse corpse)
			{
				...
				{
					IngestedCalculateAmounts(corpse, parent.pawn, Props.minNutritionGain, neededNutrition, out float nutritionIngested);
		...
		public static void IngestedCalculateAmounts(Corpse corpse, Pawn ingester, float minNutritionGain, float nutritionWanted, out float nutritionIngested)
		{
			...
			{
				bodyPartNutrition = minNutritionGain;
			}
		...
		public override bool Valid(LocalTargetInfo target, bool throwMessages = false)
		{
			if (target.HasThing && target.Thing is Corpse corpse)
			{
				if (!Props.acceptableRotStages.Contains(corpse.GetRotStage()))
				{
					if (throwMessages)
					{
						Messages.Message("BMT_PL_MessageWrongRottingStage".Translate(), corpse, MessageTypeDefOf.RejectInput, historical: false);
					}
					return false;
				}
